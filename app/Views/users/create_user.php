						...
						<form role="form" id="createProfile" method="post" action="<?= site_url('user/create_user'); ?>">
							<div class="card-body">
								...
								<div class="row">
									...
									<div class="col-sm-4" id="user_program" style="display: none;">
										<div class="form-group">
											<label>Program <span class="mandatory"> * </span></label>
											<select class="form-control" name="up_mod_id">
												<option value="">Please select program</option>
												<?php foreach ($program as $key) { ?>
													<option value="<?= $key['mod_id']; ?>"><?= $key['mod_eng_name']; ?></option>
												<?php } ?>
											</select>
										</div>
									</div>
						...
						<table class="table table-striped">
							...
							<tbody>
								<?php $i=1;foreach (array_reverse($all_user) as $key) { 
									$role = (new HomeModel())->getData(array('role_id'=>$key['user_role_id']),'jinlms_user_role');
								?>
								<tr>
									<td><?= $i++; ?></td>
