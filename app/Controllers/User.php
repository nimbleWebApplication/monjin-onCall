...
class User extends Controller
{
	function create_user_details()
	{
		...
		$data = [
			...
			"user_role" => (new HomeModel())->getData(array('role_isValid'=>0),'jinlms_user_role'),
			"program" => (new HomeModel())->getData(array('mod_isDelete'=>0),'jinlms_module'),
		...
		if($this->request->getMethod() == 'post'){
			...
			if($user['user_role_id'] == 5 && !empty($this->request->getVar('up_mod_id'))){
				$program = [
					'up_user_id'=>$user_id,
					'up_mod_id'=>$this->request->getVar('up_mod_id'),
					'up_createdOn' => date('Y-m-d H:i:s'),
				];
				(new HomeModel())->insertData($program,"jinlms_user_programs");
			}
